...
class HandGestureRecognizer:
    ...
    def process_frame(self, image):
        ...
        # Gradio gives RGB; OpenCV expects BGR for processing
        ...
        # Detect
        image_rgb = cv.cvtColor(frame_bgr, cv.COLOR_BGR2RGB)
        results = self.hands.process(image_rgb)
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Track index fingertip when pointing (id==2 assumed)
                if hand_sign_id == 2 and len(landmark_list) > 8:
                    self.point_history.append(landmark_list[8])
                else:
                    self.point_history.append([0, 0])

                # Finger gesture classification (only when we have enough history)
                finger_gesture_id = 0
                ...
                # Temporal smoothing of finger gesture
                self.finger_gesture_history.append(finger_gesture_id)
                most_common_fg_id = Counter(self.finger_gesture_history).most_common()
                ...
                debug_image = self.draw_info_text(
                    ...
                    brect,
                    handedness,
                    self.safe_label(self.keypoint_classifier_labels, hand_sign_id),
                    self.safe_label(self.point_history_classifier_labels, most_common_fg_id[0][0]),
                )

                # Opportunistic sample capture
                if len(self.captured_samples) < self.max_samples:
                    # ~3% chance per frame keeps it light; tweak if needed
                    if len(self.captured_samples) == 0 or np.random.random() < 0.03:
                        self.captured_samples.append(
                            {
                                # Store as RGB for Gradio/gallery
                                "image": cv.cvtColor(copy.deepcopy(debug_image), cv.COLOR_BGR2RGB),
                                "gesture": self.safe_label(self.keypoint_classifier_labels, hand_sign_id),
                                "hand": handedness.classification[0].label,
                            }
                        )
        else:
        ...
        return cv.cvtColor(debug_image, cv.COLOR_BGR2RGB)

    # -------------- helpers --------------
    def get_captured_samples(self):
        return [s["image"] for s in self.captured_samples]

    def clear_samples(self):
        self.captured_samples = []

    @staticmethod
    def safe_label(labels, idx):
        # Avoid index errors if model returns unexpected id
        if 0 <= idx < len(labels):
            return labels[idx]
        return ""
    ...
    def pre_process_point_history(self, image, point_history):
        image_width, image_height = image.shape[1], image.shape[0]
        # Work on a list (not deque) and ensure [x, y] shape
        temp = [[int(p[0]), int(p[1])] for p in list(point_history)]
        if not temp:
            return []
        base_x, base_y = temp[0]
        out = []
        for x, y in temp:
            out.append((x - base_x) / max(image_width, 1))
            out.append((y - base_y) / max(image_height, 1))
        return out
    ...
    def draw_landmarks(self, image, landmark_point):
        ...
        for idx, (x, y) in enumerate(landmark_point):
            if idx in [0, 1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15, 17, 18, 19]:
                cv.circle(image, (x, y), 5, (255, 255, 255), -1)
                cv.circle(image, (x, y), 5, (0, 0, 0), 1)
            if idx in [4, 8, 12, 16, 20]:
                cv.circle(image, (x, y), 8, (255, 255, 255), -1)
    ...
    def draw_info_text(self, image, brect, handedness, hand_sign_text, finger_gesture_text):
        cv.rectangle(image, (brect[0], brect[1]), (brect[2], max(brect[1] - 22, 0)), (0, 0, 0), -1)
        info_text = handedness.classification[0].label
        if hand_sign_text:
            info_text = f"{info_text}:{hand_sign_text}"
        cv.putText(image, info_text, (brect[0] + 5, max(brect[1] - 4, 12)),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv.LINE_AA)
    ...
    def draw_point_history(self, image, point_history):
        for index, point in enumerate(point_history):
            x, y = point
            if x != 0 and y != 0:
                cv.circle(image, (x, y), 1 + int(index / 2), (152, 251, 152), 2)
        return image
...
def predict(image):
    """Process each frame (RGB numpy array from Gradio)."""
    if image is None:
        return None
    return recognizer.process_frame(image)
...
def show_samples():
    """Return list of RGB images for the gallery (empty list if none)."""
    samples = recognizer.get_captured_samples()
    return samples or []


def clear_and_restart():
    """Clear captured samples and clear gallery."""
    recognizer.clear_samples()
    return []


# NOTE:
# - show_api=False avoids the gradio_client JSON-schema path that throws:
#   TypeError: argument of type 'bool' is not iterable
# - analytics_enabled=False avoids network calls in restricted envs.
with gr.Blocks(
    title="Hand Gesture Recognition with Sample Capture",
    fill_height=True,
    analytics_enabled=False,
    show_api=False,
) as demo:
    gr.Markdown("# Hand Gesture Recognition with Sample Capture")
    gr.Markdown("Allow camera access to detect hand gestures in real-time. Samples are captured automatically.")

    with gr.Row():
        with gr.Column():
            webcam_input = gr.Image(sources=["webcam"], label="Live Feed")  # no streaming=True
            webcam_output = gr.Image(label="Detection Output")
    ...
    sample_gallery = gr.Gallery(label="Captured Detection Samples", columns=3, rows=2, allow_preview=True)

    # Live streaming from webcam -> predict
    webcam_input.stream(fn=predict, inputs=webcam_input, outputs=webcam_output)
...
if __name__ == "__main__":
    # Render/hosted envs often require binding to 0.0.0.0 and using provided PORT
    port = int(os.getenv("PORT", "10000"))
    demo.queue(concurrency_count=2, max_size=8)
    demo.launch(
        server_name="0.0.0.0",
        server_port=port,
        share=True,              # create a public link when localhost isn't reachable
        show_error=True,
        allowed_paths=None,      # default
    )
