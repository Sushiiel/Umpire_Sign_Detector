...
# -*- coding: utf-8 -*-
"""
Hand Gesture Recognition System - Production Ready
...
class HandGestureRecognizer:
    ...
    def process_frame(self, image):
        ...
        try:
            ...
            if results.multi_hand_landmarks is not None:
                ...
                status_message = f"Detected {num_hands} hand(s)"
                ...
                for hand_landmarks, handedness in zip(
                    ...
                    debug_image = self._draw_info_text(
                        ...
                        handedness,
                        self.keypoint_classifier_labels[hand_sign_id],
                        self.point_history_classifier_labels[most_common_gesture]
                    )
                    
                    hand_type = handedness.classification[0].label
                    gesture_name = self.keypoint_classifier_labels[hand_sign_id]
                    status_message = f"{hand_type} hand - Gesture: {gesture_name}"
                    
                    self.frame_count += 1
                    if (len(self.captured_samples) < self.max_samples and 
                        self.frame_count % 30 == 0):
                        self._capture_sample(debug_image, gesture_name, hand_type)
            else:
                self.point_history.append([0, 0])
    ...
    def _preprocess_landmark(self, landmark_list):
        ...
        max_value = max(map(abs, temp_list))
        if max_value > 0:
            temp_list = [v / max_value for v in temp_list]
        return temp_list
    ...
    def _draw_landmarks(self, image, landmarks):
        ...
        for start, end in connections:
            if start < len(landmarks) and end < len(landmarks):
                cv.line(image, tuple(landmarks[start]), tuple(landmarks[end]),
                       (0, 0, 0), 6)
                cv.line(image, tuple(landmarks[start]), tuple(landmarks[end]),
                       (255, 255, 255), 2)
        ...
        for i, point in enumerate(landmarks):
            if i in [4, 8, 12, 16, 20]:
                cv.circle(image, tuple(point), 8, (255, 255, 255), -1)
                cv.circle(image, tuple(point), 8, (0, 0, 0), 1)
            else:
    ...
    def _draw_info_text(self, image, brect, handedness, gesture, finger_gesture):
        cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 22),
                    (0, 0, 0), -1)
        ...
        if finger_gesture:
            cv.putText(image, f"Movement: {finger_gesture}", (10, 60),
                      cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 4, cv.LINE_AA)
            cv.putText(image, f"Movement: {finger_gesture}", (10, 60),
                      cv.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2, cv.LINE_AA)
    ...
    def _draw_point_history(self, image, point_history):
        for i, point in enumerate(point_history):
            if point[0] != 0 and point[1] != 0:
                radius = 1 + int(i / 2)
                cv.circle(image, tuple(point), radius, (152, 251, 152), 2)
        return image
...
def clear_samples():
    ...
    if recognizer is not None:
        recognizer.clear_samples()
    return None, "Samples cleared successfully!"


def create_interface():
    with gr.Blocks(title="Hand Gesture Recognition") as demo:
        gr.Markdown("# Hand Gesture Recognition System")
        gr.Markdown("""
        Real-time hand gesture detection using MediaPipe and deep learning.
        
        **Instructions:**
        1. Allow camera access when prompted
        2. Position your hand in front of the camera
        3. Try different gestures
        4. Samples are captured automatically
        """)
        ...
        with gr.Row():
            show_samples_btn = gr.Button("Show Samples", variant="primary")
...
if __name__ == "__main__":
    ...
    # For Render deployment - use share=False but allow server_name binding
    demo.launch(
        server_name="0.0.0.0",
        server_port=port,
        share=False,
